...
export const AppShell = React.forwardRef<HTMLDivElement, AppShellProps>(
  ({ sidebar, header, children, footer, className }, ref) => {
    const [sidebarOpen, setSidebarOpen] = useState(false);

    return (
      <div
        ref={ref}
        className={cn('flex h-screen flex-col bg-slate-50 dark:bg-slate-950', className)}
      >
        {/* Header */}
        {header && (
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {sidebar && (
            <>
              ...
              <aside
                className={cn(
                  'absolute inset-y-0 left-0 w-64 border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 sm:static sm:w-64',
