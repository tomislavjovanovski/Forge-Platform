...
export const ThemeSwitcher = React.forwardRef<HTMLDivElement, ThemeSwitcherProps>(
  ({ theme: controlledTheme, onChange, className }, ref) => {
    ...
    useEffect(() => {
      ...
      const initialTheme = controlledTheme ?? preferred;
    ...
    if (!mounted) {
      return (
        <div ref={ref} className={cn('inline-flex gap-1 rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-900', className)}>
