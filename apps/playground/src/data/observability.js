...
function cycleIndex(items, index) {
    return items[index % items.length];
}
function getRealtimePoint(index) {
    return {
        label: realtimeLabels[index],
        throughput: 930 +
            Math.round(Math.sin(index / 2.4) *
                70 +
                index * 4),
        latency: 170 +
            Math.round(Math.cos(index / 1.7) *
                22 +
                index * 1.8),
        ...
        saturation: 58 +
            Math.round(Math.cos(index / 2.8) *
                9 +
                index),
    };
}
function getUserAnalyticsPoint(index) {
    return {
...
export function advanceDashboardSnapshot(snapshot, tick) {
    const nextStreamTick = snapshot.streamTick + 1;
    const lastRealtime = snapshot.realtimeMetrics[snapshot.realtimeMetrics
        .length - 1];
    const nextRealtimeLabel = `${9 +
        Math.floor((24 + tick * 2) / 60)}:${String((24 + tick * 2) % 60).padStart(2, '0')}`;
    const nextRealtime = {
        label: nextRealtimeLabel,
        throughput: lastRealtime.throughput +
            (tick % 2 === 0
                ? 32
                : -18),
        latency: Math.max(132, lastRealtime.latency +
            (tick % 3 === 0
                ? 12
                : -7)),
        errorRate: Number(Math.max(0.18, lastRealtime.errorRate +
            (tick % 4 === 0
                ? 0.09
                : -0.03)).toFixed(2)),
        saturation: Math.min(89, Math.max(44, lastRealtime.saturation +
            (tick % 2 === 0
                ? 4
                : -3))),
    };
    ...
    const lastUsers = snapshot.userAnalytics[snapshot.userAnalytics
        .length - 1];
    const nextUserAnalytics = [
        ...snapshot.userAnalytics.slice(1),
        {
            label: nextUserLabel,
            activeUsers: lastUsers.activeUsers +
                (tick % 2 === 0
                    ? 210
                    : -90),
            retainedUsers: lastUsers.retainedUsers +
                (tick % 2 === 0
                    ? 160
                    : -70),
            conversions: Math.max(210, lastUsers.conversions +
                (tick % 3 ===
                    0
                    ? 16
                    : -8)),
    ...
    const nextDeployments = snapshot.deployments.map((deployment, index) => {
        if (index === 1) {
            return {
                ...deployment,
                canaryPercent: Math.min(100, deployment.canaryPercent +
                    10),
                status: deployment.canaryPercent >=
                    90
                    ? 'healthy'
                    : 'monitoring',
            };
        ...
        if (index === 3) {
            return {
                ...deployment,
                incidents: Math.max(1, deployment.incidents -
                    1),
                canaryPercent: Math.min(30, deployment.canaryPercent +
                    4),
                status: deployment.incidents <=
                    2
                    ? 'monitoring'
                    : deployment.status,
            };
    ...
    const trimmedPipelines = nextPipelines
        .slice(0, 28)
        .map((run, index) => {
        if (index === 1 &&
            run.status ===
    ...
    const nextTraceSpans = snapshot.traceSpans.map((span, index) => {
        ...
        if (span.service ===
            ...
            return {
                ...span,
                duration: Math.max(10, span.duration +
                    (tick % 2 ===
                        0
                        ? -2
                        : 3)),
